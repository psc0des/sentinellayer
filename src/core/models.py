...
# ============================================
# Playbook (Phase 34D — Tier 3 Playbook Generator)
# ============================================

class Playbook(BaseModel):
    """Tier 3 remediation playbook for a governance decision.

    Generated by ``PlaybookGenerator`` for action+resource combinations that
    have no Tier 1 SDK tool.
    """
    action_type: str
    resource_id: str
    az_command: str                # display-friendly full command string
    executable_args: list[str]     # argv form for safe subprocess invocation (Phase E)
    rollback_command: Optional[str] = None   # None for idempotent or irreversible ops
...
class AzPlaybookExecution(BaseModel):
    ...
    Written to ``governance-executions`` Cosmos container (or ``data/az_executions/``
    in mock mode) on every call — live, dry-run, and rejected.

    ``record_type = "az_execution"`` distinguishes these from
    ``ExecutionRecord`` (SDK execution) documents in the same Cosmos container.
    ``ExecutionGateway._ensure_loaded()`` skips records with this tag.
    """
    execution_id: str
    record_type: str = "az_execution"  # discriminator — never change
    decision_id: str
    resource_id: str                   # partition key
    action_type: str
    az_command: str                    # display command (for audit readability)
    executable_args: list[str]         # exact argv used (or would have been used)
    mode: Literal["live", "dry_run"]
    approved_by: str
    allowlist_matched: bool
    exit_code: Optional[int] = None    # None when dry_run with no --what-if
    stdout: str = ""
    stderr: str = ""
