...
| `ExecutionAgent` | Plans and executes approved governance actions | **Plan phase** — 5 tools: `get_resource_details`, `list_nsg_rules`, `query_metrics`, `fetch_azure_docs` (ARM provider metadata → correct api_version), `submit_execution_plan`. 4-step decision tree: (1) specific tool (NSG/VM/AppService/AKS/Storage), (2) `update_resource_property` for any property PATCH, (3) `guided_manual` — copy-pasteable az CLI commands + numbered Portal steps + doc URL, (4) `manual` fallback. Plan includes `remediation_confidence` field (`auto_fix`/`generic_fix`/`guided_manual`/`manual`) computed by `_compute_confidence()` — worst-confidence-wins for mixed plans. **Execute phase (Phase 34A) — 15 tools**: 7 original write tools + 5 new Tier 1 SDK tools (`restart_app_service`, `restart_function_app`, `scale_app_service_plan`, `scale_aks_nodepool`, `rotate_storage_keys`) + `fetch_azure_docs` + `update_resource_property` + `report_step_result`. Each new tool accepts `dry_run: bool = False`; `execute()` gains `dry_run: bool = False` parameter; result includes `mode: "live"\|"dry_run"`. Mock plan: infers `update_resource_property` from 7 known reason patterns; unknown `UPDATE_CONFIG` and `CREATE_RESOURCE` → `guided_manual` (not `manual`). Dashboard renders `RemediationConfidenceBadge` beside plan summary; `GuidedManualSteps` renders CLI block (copy button) + Portal checklist + doc link. |
...
- **Production logging** — `logging.basicConfig(level=logging.INFO, force=True)` runs **inside the FastAPI `lifespan` startup hook** in `dashboard_api.py`. Module-import placement is not enough on its own: uvicorn applies its own dictConfig *after* module import and overrides the root handler. The lifespan hook runs after uvicorn has fully settled its logging, so calling `basicConfig` again there with `force=True` wins. The `_RequestIDLogFilter` is re-attached to the new handlers in the same hook. Without this, every app-level `logger.info/warn/error` is dropped in production and only uvicorn access logs reach Container Apps log analytics — which is what made scans appear silent in the 2026-04-27 incident.
