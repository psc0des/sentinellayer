...
- **Production logging** — `logging.basicConfig(level=logging.INFO, force=True)` runs at module import in `dashboard_api.py`. Module-import placement (vs. the `__main__` guard, where it used to live) ensures app logs reach stdout under the container's `CMD ["uvicorn", ...]` launch path. Without this, every `logger.info/warn/error` was dropped in production and only uvicorn access logs reached Container Apps log analytics — making stuck scans appear silent.
