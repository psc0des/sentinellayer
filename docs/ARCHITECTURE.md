...
- **Rollback** — after a fix is applied (`status=applied`), `ExecutionGateway.rollback_agent_fix()` calls `ExecutionAgent.rollback()`. On success: sets `status → rolled_back`, stores `rollback_log`. On failure: keeps `status = applied` (fix is still in place), stores failed `rollback_log`, appends failure note. Mock path maps each `ActionType` to its deterministic inverse (RESTART→deallocate, SCALE_UP/DOWN→resize back, NSG→restore rule, DELETE→cannot auto-rollback). Live path is LLM-driven using `rollback_hint` from the stored plan. `rollback_hint` is action-type-specific and captures before-state at plan time: NSG deletes embed the full original rule properties (priority, protocol, port, source/dest) as JSON; VM resizes embed the current SKU; property updates embed the current value; deletes flag irreversibility. `_PLAN_INSTRUCTIONS` enforce this for all mutating operation types. Both `create_nsg_rule` tool instances (execute and rollback phases) use identical SDK-native parameter names (`source_address_prefix`, `destination_address_prefix`, `destination_port_range`) — a mismatch between the two previously caused rollback calls to be rejected by Azure. On LLM/framework exception, `rollback()` returns `{"success": False, ...}` — it does **not** fall back to the mock (mock returning `success=True` would falsely mark the record `rolled_back` while Azure never ran). The dashboard shows an amber ↩ Rollback button next to the Applied badge; on failure a rose-colored "Rollback attempted but failed" banner shows even when `rollback_log` is empty (e.g. API error or LLM timeout), and the button remains for retry.
...
- **Deploy agent proposal guard** — `_AGENT_INSTRUCTIONS` only instructs the agent to propose when a violation is confirmed. A resource that is already correctly configured produces no proposal. Previously the instructions said "propose for EVERY finding", causing the LLM to propose on compliant resources with reason text like "No action needed".
