...
| POST | `/api/execution/{execution_id}/dismiss` | Human dismisses a verdict. **Phase 35A:** writes `VerdictOverride(DISMISS_APPROVED or DISMISS_ESCALATED)` to `governance-overrides`. |
...
| POST | `/api/execution/{execution_id}/condition/{condition_index}/satisfy` | Human marks a condition on a `conditional` execution record as satisfied. Body: `{satisfied_by: string, note?: string}`. Triggers `try_execute_if_all_satisfied()` — promotes record to `manual_required` if all conditions are now met. **Phase 35A:** writes `VerdictOverride(SATISFY_CONDITION)` to `governance-overrides`. |
...
| POST | `/api/execution/{execution_id}/force-execute` | Admin bypass — skip all unsatisfied conditions and promote to `manual_required`. Body: `{admin_user: string, justification: string}`. Justification must be **≥20 characters** (raises 400 otherwise). Writes `force_executed_by`, `force_executed_at`, `force_execute_justification` to the record. **Phase 35A:** writes `VerdictOverride(FORCE_EXECUTE)` to `governance-overrides`. |
| GET | `/api/execution/{execution_id}/terraform` | Generate Terraform HCL fix for a `manual_required` or `pr_created` execution record |
| GET | `/api/workflow/diagram` | Returns the governance workflow topology as a Mermaid `flowchart TD` string. Generated by `agent_framework.WorkflowViz.to_mermaid()`; topology is immutable so result is cached at module load. Response: `{format: "mermaid", diagram: string}`. Used by the Overview page's "How RuriSkry Decides" section. |
...
| POST | `/api/decisions/{decision_id}/validate` | **Phase 34F — A2 Validator.** Body: `{"resolved_call": {"argv": [...]}}`. Calls the A2 Validator LLM (5s hard timeout). Returns a `ValidatorBrief`: `{brief_id, summary, caveats, risk_level, raw_text, validator_status}`. `validator_status` is `"ok"`, `"unavailable"`, or `"timeout"` — always HTTP 200. If `resolved_call` is empty, playbook is auto-resolved from the decision. |
| POST | `/api/decisions/{decision_id}/playbook/execute` | **Phase 34E/34F — Execute playbook.** Body: `{"mode": "live"\|"dry_run", "approved_by": string, "validator_brief_id": string?, "validator_brief_summary": string?, "validator_brief_caveats": list?}`. Runs the playbook command through the audited az CLI executor (allowlist-validated, `shell=False`, hard timeout). Allowlist covers: webapp/functionapp restart, storage/keyvault/cosmosdb/servicebus update, network nsg rule update, and vm restart/update/resize/delete. Returns an `AzPlaybookExecution` audit record with `exit_code`, `stdout`, `stderr`, `duration_ms`, `execution_id`, and optional `validator_brief_*` fields for postmortem linkage. Returns `403` if command doesn't match the allowlist, `404` if no playbook/decision found. |
| GET | `/api/overrides` | **Phase 35C — Override history.** List recent `VerdictOverride` records, newest-first. Query params: `limit` (1–500, default 50), `action_type` (e.g. `restart_service`), `override_type` (e.g. `force_execute`). Cosmos internal fields (`_rid`, `_ts`, `_etag`, `id`) are stripped. Returns `{"count": N, "overrides": [...]}`. |
| GET | `/api/overrides/metrics` | **Phase 35C — Override metrics.** Aggregate stats across all stored operator overrides. Returns `{"total": N, "by_override_type": {...}, "top_action_types": [{"action_type": "...", "count": N}, ...], "most_overridden_verdict": "..."}`. Used by the Overview dashboard OverrideMetricsCard. |
| GET | `/api/coverage/status` | **Phase 40 — Coverage preflight.** Returns result of async preflight checks against 5 Microsoft APIs (resource_graph, advisor, policy_insights, defender, resource_health). Each API entry: `{ok: bool, latency_ms: float?, error: string?}`. Also includes `rules: {total, by_category}`. Response is TTL=300s cached; `invalidate()` called on inventory refresh. Rendered as the `CoverageStatusBanner` on the Agents page. |
