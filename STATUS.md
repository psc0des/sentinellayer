...
**Last updated:** 2026-04-06 (Four rollback fixes: (1) **Rollback credential bug** — rollback tools in `_rollback_with_framework` imported sync `DefaultAzureCredential` from outer scope and used it with `async with`, which fails because the sync credential has no `__aenter__`/`__aexit__`; execute-phase tools correctly use `from azure.identity.aio import DefaultAzureCredential as AioCredential`; rollback tools now match; (2) **Rollback missing plan** — `rollback_agent_fix` raised `ValueError` (→ HTTP 400) when `execution_plan` was None (e.g. redeployment between execute and rollback caused Cosmos reload without the plan); now generates plan on-the-fly like `execute_agent_fix` does; (3) **Stale execution status in AlertFindingActions** — component initialized `execStatus` from alert record's `execution_status` field which is set at investigation time and never updated; after redeployment, component showed `applied` (React state) but backend had `manual_required` (Cosmos); `AlertFindingActions` now fetches real status from new `GET /api/execution/{id}/record` endpoint on mount; (4) **api.js error detail** — `rollbackAgentFix` always threw generic "rollback failed" regardless of backend `detail`; now reads `body.detail` from error response; **793 tests passing, 0 failed**)

**Last updated (prev):** 2026-04-06 (Three fixes: (1) **Live investigation log + panel Investigate button** — `AlertPanel` drilldown now has an "Investigate this alert" button (for `pending` alerts) alongside the table-row button; opening the panel mid-investigation shows a real-time terminal-style "Investigation Log" by polling `GET /api/alerts/{id}/status` every 1.5s; backend `_emit_alert_event()` appends each SSE event to `investigation_log` field on the alert record so the log is always recoverable (no SSE queue competition); `fetchAlertStatus` added to `api.js`; (2) **Rollback silent-failure bug** — `ExecutionAgent.rollback()` was falling back to `_rollback_mock()` on any LLM exception, which returned `success=True` — the record was marked `rolled_back` in Cosmos but Azure never ran `vm deallocate`; fixed to return `{"success": False, ...}` on exception (matching the `execute()` handler pattern); (3) **Rollback empty-log UI gap** — failure panel in `Alerts.jsx` and `EvaluationDrilldown.jsx` was gated on `rollback_log.length > 0`, so any failure that produced zero steps (API 4xx/5xx, LLM called no tools) was silently swallowed — spinner stopped, no message shown; fixed: failure panel now shows whenever `rollbackResult` exists and status is still `applied`, with contextual message (steps if present, else API error string, else generic retry prompt); **793 tests passing, 0 failed**)

**Last updated (prev):** 2026-03-14 (Agents page bug fixes: (1) **Modal state lost on menu close** — `panel` state lived inside `CardMenu` component; clicking a menu item called `onClose()` which unmounted `CardMenu` before the modal could render, so History/Last Run Results/Agent Details modals were silently no-ops; fix: moved `panel` state up to `AgentCard` which wraps `GlowCard` + modals in a React fragment — menu close no longer destroys panel state; (2) **Unicode escapes rendered as literal text in JSX** — `\u2715` (✕ close button) and similar escapes in JSX text content are treated as literal characters, not Unicode; fix: replaced with actual Unicode characters (`✕`, `—`, `·`, `→`) or `{'\uXXXX'}` JSX expressions; (3) **Refresh resilience two-layer restore** — `fetchAgentLastRun` reads in-memory backend state which is lost on container restart/revision swap; `useScanManager` now has a second restore layer using `fetchScanHistory(50)` (Cosmos-backed, survives restarts) as fallback for any agent whose running scan wasn't found by layer 1; **793 tests passing, 0 failed**)

**Last updated (prev):** 2026-03-14 (Enterprise Agents Page Redesign: **Single-system architecture replaces scattered scan state.** New `useScanManager` custom hook owns ALL scan state (start/stop/poll/restore-on-refresh) — previously split across `ConnectedAgents` + `AgentControls`. New `AgentCardGrid` component provides agent cards with inline scan/stop/live log/last run buttons. New `ScanLogViewer` is a dual-mode log viewer (live SSE for running scans, historical structured display for completed scans) replacing `LiveLogPanel`. New `ScanHistoryTable` is a Cosmos-backed scan history table with agent/status filters. `Agents.jsx` rewritten as a clean composition: `useScanManager` + `AgentCardGrid` + `ScanHistoryTable` + `ScanLogViewer`. Old components (`ConnectedAgents.jsx`, `AgentControls.jsx`, `LiveLogPanel.jsx`) retained but no longer imported by `Agents.jsx`; **793 tests passing, 0 failed**)

**Last updated (prev):** 2026-03-14 (Two frontend bug fixes: (1) **Dashboard slow load** — `load()` in `App.jsx` was blocking `<LoadingScreen>` until all 6 API calls resolved (including `fetchEvaluations(200)`, `fetchScanHistory(200)`, `fetchAlerts(200)`, `fetchPendingReviews()` — all Cosmos queries); after the Cosmos execution persistence deployment, `fetchPendingReviews()` became a Cosmos call (was instant filesystem read), making the blocking time much longer; fix: phase 1 calls only `fetchMetrics()` + `fetchAgents()` (fast, no Cosmos) → clears `LoadingScreen`; phase 2 calls `fetchAll()` non-blocking immediately after page renders (populates evaluations/scans/alerts/pendingReviews within 1-2s, 5s background refresh continues as before); (2) **Live scan log not visible after Agents+Scans merge** — `LiveLogPanel` uses `position: fixed` but was rendering inside `ConnectedAgents` whose `GlowCard` components have `backdropFilter: blur(12px)` — this creates a CSS stacking context that can confine fixed-position children to a local containing block rather than the viewport; fix: added `createPortal(children, document.body)` in `LiveLogPanel.jsx` so both panel instances (from `AgentControls` and `ConnectedAgents`) render directly on `document.body`, completely bypassing any CSS ancestor containment; **793 tests passing, 0 failed**)
...
| AgentControls dashboard panel (Phase 13) | ✅ Superseded | Replaced by `useScanManager` hook + `AgentCardGrid` component in Agents page redesign |
...
AZURE_OPENAI_ENDPOINT=https://ruriskry-foundry-<suffix>.cognitiveservices.azure.com/
AZURE_OPENAI_DEPLOYMENT=gpt-41
AZURE_SEARCH_ENDPOINT=https://ruriskry-search-<suffix>.search.windows.net
AZURE_SEARCH_INDEX=incident-history     ← seeded with 7 incidents
COSMOS_ENDPOINT=https://ruriskry-cosmos-<suffix>.documents.azure.com:443/
COSMOS_DATABASE=ruriskry
COSMOS_CONTAINER_DECISIONS=governance-decisions
AZURE_KEYVAULT_URL=https://ruriskry-kv-<suffix>.vault.azure.net/
