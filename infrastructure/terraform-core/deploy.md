...
| Azure AI Foundry | `ruriskry-core-foundry-<suffix>` | gpt-4.1-mini LLM (version 2025-04-14, Standard, 150K TPM — configurable via tfvars) |
...
| DeployAgent Defender/Policy safety nets return empty results | Container App MI lacks permissions to call `Microsoft.Security/assessments` or `Microsoft.PolicyInsights/policyStates` | Both are covered by the `Reader` role at subscription scope (`azurerm_role_assignment.subscription_reader`). If the role is missing, run `terraform apply` in `terraform-core`. Agents gracefully degrade (log a warning, return empty list) — scans still complete. |
| "Scan log unavailable (backend was restarted while scan was running)" on every new scan | Container App running multiple replicas without sticky sessions — scan starts on Replica A (SSE queue created there) but browser SSE stream is load-balanced to Replica B/C (no queue) | `deploy.sh` sets sticky sessions automatically (Step 4a-ii). `sticky_sessions_affinity` was removed from the azurerm provider in ~4.63.0 and is no longer Terraform-manageable. If you deployed before this fix or are enabling manually: `az containerapp ingress sticky-sessions set --name ruriskry-core-backend-<suffix> --resource-group ruriskry-core-engine-rg --affinity sticky` |
...
| Azure AI Foundry (gpt-4.1-mini Standard, 150K TPM default) | Pay-per-token |
