...
class TestPolicyComplianceAgent:
    ...
    async def test_compliant_action_returns_zero_score(self, agent):
    ...
    async def test_returns_policy_result_model(self, agent):
        action = _make_action()
        result = await agent.evaluate(action, now=_WEDNESDAY_NOON)
        assert isinstance(result, PolicyResult)
    ...
    async def test_pol_dr001_delete_dr_resource(self, agent):
    ...
    async def test_pol_dr001_scale_down_dr_resource(self, agent):
    ...
    async def test_pol_dr001_scale_up_dr_resource_allowed(self, agent):
    ...
    async def test_pol_dr001_severity_is_critical(self, agent):
    ...
    async def test_pol_sec001_modify_nsg(self, agent):
    ...
    async def test_pol_sec001_delete_nsg(self, agent):
    ...
    async def test_pol_sec001_non_nsg_resource_not_blocked(self, agent):
    ...
    async def test_pol_sec001_severity_is_high(self, agent):
        action = _make_action(
            action_type=ActionType.MODIFY_NSG,
            resource_type="Microsoft.Network/networkSecurityGroups",
        )
    ...
    async def test_pol_chg001_triggers_during_weekend_window(self, agent):
        ...
        result = await agent.evaluate(action, now=_SATURDAY_NOON)
        assert any(v.policy_id == "POL-CHG-001" for v in result.violations)

    async def test_pol_chg001_not_triggered_outside_window(self, agent):
    ...
    async def test_pol_chg001_triggers_friday_at_window_start(self, agent):
        """Friday at exactly 17:00 UTC is the first blocked moment."""
        action = _make_action(
            resource_id="/subscriptions/x/resourceGroups/prod/providers/Microsoft.Compute/vm/api",
        )
        result = await agent.evaluate(action, now=_FRIDAY_AT_START)
        assert any(v.policy_id == "POL-CHG-001" for v in result.violations)

    async def test_pol_chg001_not_triggered_friday_before_window(self, agent):
        """Friday at 16:59 is just before the window — should be allowed."""
        action = _make_action(
            resource_id="/subscriptions/x/resourceGroups/prod/providers/Microsoft.Compute/vm/api",
        )
        result = await agent.evaluate(action, now=_FRIDAY_BEFORE)
        assert not any(v.policy_id == "POL-CHG-001" for v in result.violations)

    async def test_pol_chg001_triggers_monday_before_end(self, agent):
        """Monday 07:59 UTC is still within the blocked window."""
        action = _make_action(
            resource_id="/subscriptions/x/resourceGroups/prod/providers/Microsoft.Compute/vm/api",
        )
        result = await agent.evaluate(action, now=_MONDAY_BEFORE_END)
        assert any(v.policy_id == "POL-CHG-001" for v in result.violations)

    async def test_pol_chg001_not_triggered_monday_at_end(self, agent):
        """Monday 08:00 UTC the window has closed — changes are allowed again."""
        action = _make_action(
            resource_id="/subscriptions/x/resourceGroups/prod/providers/Microsoft.Compute/vm/api",
        )
        result = await agent.evaluate(action, now=_MONDAY_AT_END)
        assert not any(v.policy_id == "POL-CHG-001" for v in result.violations)

    async def test_pol_chg001_dev_environment_not_blocked(self, agent):
    ...
    async def test_pol_cost001_triggers_on_high_savings(self, agent):
    ...
    async def test_pol_cost001_triggers_on_delete_resource_cost(self, agent):
    ...
    async def test_pol_cost001_not_triggered_below_threshold(self, agent):
    ...
    async def test_pol_cost001_not_triggered_at_threshold(self, agent):
    ...
    async def test_pol_cost001_not_triggered_without_cost_data(self, agent):
    ...
    async def test_pol_crit001_delete_critical_resource(self, agent):
    ...
    async def test_pol_crit001_restart_critical_resource(self, agent):
    ...
    async def test_pol_crit001_scale_up_critical_resource_allowed(self, agent):
    ...
    async def test_pol_crit001_non_critical_tag_not_blocked(self, agent):
    ...
    async def test_score_zero_for_no_violations(self, agent):
        action = _make_action(action_type=ActionType.SCALE_UP)
        result = await agent.evaluate(action, now=_WEDNESDAY_NOON)
        assert result.sri_policy == 0.0

    async def test_score_40_for_single_critical_violation(self, agent):
        action = _make_action(action_type=ActionType.DELETE_RESOURCE)
        result = await agent.evaluate(
            action,
            resource_metadata={"tags": {"disaster-recovery": "true"}},
    ...
    async def test_score_compounds_across_violations(self, agent):
    ...
    async def test_score_capped_at_100(self, agent):
    ...
    async def test_total_policies_checked_matches_loaded(self, agent):
        action = _make_action()
        result = await agent.evaluate(action, now=_WEDNESDAY_NOON)
        assert result.total_policies_checked == len(agent._policies)

    async def test_policies_passed_plus_violations_equals_total(self, agent):
        action = _make_action(action_type=ActionType.DELETE_RESOURCE)
        result = await agent.evaluate(
            action,
            resource_metadata={"tags": {"disaster-recovery": "true"}},
    ...
    async def test_reasoning_mentions_violation_policy_id(self, agent):
        action = _make_action(action_type=ActionType.MODIFY_NSG,
                              resource_type="Microsoft.Network/networkSecurityGroups")
        result = await agent.evaluate(action, now=_WEDNESDAY_NOON)
        assert "POL-SEC-001" in result.reasoning

    async def test_reasoning_mentions_compliant_when_clean(self, agent):
        action = _make_action(action_type=ActionType.SCALE_UP)
        result = await agent.evaluate(action, now=_WEDNESDAY_NOON)
    ...
    async def test_infer_production_from_resource_id(self, agent):
    ...
    async def test_infer_production_from_resource_group(self, agent):
        action = _make_action(
    ...
    async def test_metadata_environment_overrides_inference(self, agent):
    ...
    async def test_custom_policies_path(self, tmp_path):
    ...
    async def test_empty_policies_returns_zero_score(self, tmp_path):
